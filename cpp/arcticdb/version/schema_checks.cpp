...
// An empty index describes zero rows, so it constrains nothing and takes the type of whatever it is combined with.
...
void check_index_types_combinable(
    ...
    const bool empty_series_written_as_timeseries = frame.norm_meta.has_series() &&
                                                    old_idx_kind == IndexDescriptor::Type::TIMESTAMP &&
                                                    new_idx_kind == IndexDescriptor::Type::ROWCOUNT;
    normalization::check<ErrorCode::E_INCOMPATIBLE_INDEX>(
            get_common_index_type(old_idx_kind, new_idx_kind) != IndexDescriptor::Type::UNKNOWN ||
                    empty_series_written_as_timeseries,
...
void align_rowcount_index_start(const TimeseriesDescriptor& existing_tsd, const pipelines::InputFrame& new_frame) {
    if (existing_tsd.index().type() != IndexDescriptor::Type::ROWCOUNT ||
        new_frame.desc().index().type() != IndexDescriptor::Type::ROWCOUNT) {
        return;
    }
    update_rowcount_normalization_data(existing_tsd.normalization(), new_frame.norm_meta, existing_tsd.total_rows());
}

std::string_view normalization_operation_str(NormalizationOperation operation) { return operation_name(operation); }
} // namespace
...
entity::OutputSchema combine_schema_with_frame(
    ...
    align_rowcount_index_start(existing_tsd, new_frame);
    if (existing_tsd.total_rows() == 0) {
        check_index_types_combinable(operation, existing_tsd, new_frame);
    ...
    try {
        return combine_schema(schemas, append_or_update_options(dynamic_schema, operation));
    } catch (const SchemaException& e) {
        // The combine says which column disagreed and why, but knows nothing of the symbol or of the two descriptors
        // as wholes. Both are what makes a descriptor mismatch diagnosable, and the shape of this message is what
        // _store.py::_diff_long_stream_descriptor_mismatch reads to show the user a column diff.
        throw StreamDescriptorMismatch(
                e.what(), new_frame.desc().id(), existing_tsd.as_stream_descriptor(), new_frame.desc(), operation
        );
    }
}
} // namespace arcticdb
