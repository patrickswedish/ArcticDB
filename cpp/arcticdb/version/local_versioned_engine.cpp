...
// Drop the symbols that have no rows to contribute, so that neither their columns nor their index type reach the
// joined schema. This matches append, where a zero-row frame is a metadata-only version bump. The test is by row
// count rather than index type on purpose: the common empty dataframe has a timestamp index with zero rows, not an
// empty one, so an index-type test would miss it.
//
// If every symbol is rowless there is nothing to prefer, so they are all kept and joined as before - a join of
// nothing but empty symbols still has to produce an empty frame with their columns.
